...
import { X, Zap, Image as ImageIcon, HelpCircle } from 'lucide-react';
...
const WeighingCamera = () => {
    ...
    const capture = () => {
        if (videoRef.current && canvasRef.current) {
            ...
            stopCamera();
            // Store captured image in sessionStorage for the confirm screen
            sessionStorage.setItem('weighing_photo', imageData);
            navigate(`/driver/trip/${id}/weighing/confirm`);
        } else {
            // Fallback: navigate without image
            navigate(`/driver/trip/${id}/weighing/confirm`);
        }
    };
    ...
    const handleGallery = () => {
        ...
        input.onchange = (e) => {
            ...
            if (file) {
                ...
                reader.onload = (ev) => {
                    sessionStorage.setItem('weighing_photo', ev.target.result);
                    stopCamera();
                    navigate(`/driver/trip/${id}/weighing/confirm`);
                };
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <button onClick={handleGallery} style={{
                        ...
                        display: 'flex', flexDirection: 'column',
                        alignItems: 'center', justifyContent: 'center',
                        color: 'white', gap: '2px', cursor: 'pointer'
                    }}>
                        <ImageIcon size={20} />
                        <span style={{ fontSize: '0.5rem', fontWeight: '600', letterSpacing: '0.05em' }}>GALERÍA</span>
                    </button>
                    ...
                    <button style={{
                        ...
                        <HelpCircle size={20} />
                        <span style={{ fontSize: '0.5rem', fontWeight: '600', letterSpacing: '0.05em' }}>AYUDA</span>
                    </button>
                </div>
            </div>
        </div>
